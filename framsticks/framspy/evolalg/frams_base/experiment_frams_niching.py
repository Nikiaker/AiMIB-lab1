...
class ExperimentFramsNiching(ExperimentFrams, ExperimentNiching):
    def __init__(self, frams_lib, optimization_criteria, hof_size, popsize, constraints, normalize, dissim, fit, genformat, archive_size, save_only_best, knn_niching, knn_nslc, crowding_dissim=None) -> None:
        ...
        ExperimentNiching.__init__(self, hof_size=hof_size,
                                   ...
                                   knn_nslc=knn_nslc,
                                   crowding_dissim=crowding_dissim
                                   )
        self.dissim = dissim
    ...
    def initialize_evolution(self, genformat, initialgenotype):
        ...
        if self.fit == "nslc":
            self.do_nslc_dissim(self.population_structures, self.population_structures.population)

    def dissimilarity(self, population):
        return self.frams_lib.dissimilarity([i.genotype for i in population], self.dissim)
    
    def crowding_distance_dissimilarity(self, population):
        return self.frams_lib.dissimilarity([i.genotype for i in population], self.crowding_dissim)
